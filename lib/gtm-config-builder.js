...
//
// Numeric fields (event_time, value, num_items) are embedded unquoted.
// They always have numeric defaults from the web container, so the
// resulting JSON body is always valid.
//
...
function _metaBody(eventName) {
  return (
    ...
    +   '"client_ip_address":"{{ET - Header client_ip}}",'
...
function _tiktokBody(eventName) {
  ...
  return (
    ...
    +     '"external_id":"{{ET - up external_id}}"'
    +   '},'
    +   '"ip":"{{ET - Header client_ip}}",'
    +   '"user_agent":"{{ET - Header user_agent}}",'
    +   '"ad":{"callback":"{{ET - ep ttclid}}"},'
    +   '"page":{"url":"{{ET - page_location}}","referrer":"{{ET - page_referrer}}"}'
    + '}'
    + '}'
  );
}
